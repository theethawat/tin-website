...
export default function MDXComponent({ children }) {
  const renderer = {
    ...
    heading: ({ level, node, children }) => {
      if (level === 1)
        return (
          <h1 className='text-2xl font-semibold font-display my-2'>
            {" "}
            {children}{" "}
          </h1>
        );
      if (level === 2)
        return (
          <h2 className='text-xl font-medium font-display my-1 '>
            {" "}
            {children}{" "}
          </h2>
        );
      if (level === 3)
        return (
          <h3 className='text-lg font-dismono font-semibold my-2'>
            {" "}
            {children}{" "}
          </h3>
        );
